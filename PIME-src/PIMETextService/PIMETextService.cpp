...
int TextService::candFontHeight() {
	...
	int dpi = 96;
	// use the candidate window's actual DPI (sharp on scaled displays)
	if (candidateWindow_ && candidateWindow_->hwnd()) {
		dpi = (int)GetDpiForWindow(candidateWindow_->hwnd());
	}
	else {
		HDC hdc = GetDC(NULL);
		if (hdc)
	...
	candFontHeight_ = -MulDiv(candFontSize_, dpi, 72);
